...
$$
$$
*\ log(1-\frac{1}{(1+exp(\beta_0 + \beta_1 * x_i))})
$$
...
::: {.callout-note icon="lightbulb"}
We fit our model twice:
Once with win-fraction-based skill metric. Once withthe point-differential based metric. We found that skill measured by point differential better matched who actually won March Madness 2024 (UConn was out model's #2 pick; they won).
:::
